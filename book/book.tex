...
本章讲述另一个内置类型，字典。
字典作为Python最有特色的功能之一;  使其成为了诸多高校优雅算法的基石。
...
\end{verbatim}
%
这行代码创建了一个表示从键\verb"'one'"到值\verb"'uno'"的映射的项。
如果你再次打印字典，可以看到一个键值对，其键值用冒号隔开:
...
\end{verbatim}
%
若要确定值是否存在于字典中，可以用返回值的集合的{\tt values}方法，
然后使用{\tt in}运算符:
\index{values method}
\index{method!values}
...
\end{verbatim}
%
{\tt in} 运算符在列表和字典中使用的算法不同。
对于列表来说，它会如章节~\ref{find}中一样，按照顺序搜索列表中的元素。
随着列表变长，确定目标的耗时也会更长。

Python中的字典则使用了叫做{\bf 哈希表}的数据结构，使其具有了一个神奇特性: 
无论字典中有多少项，{\tt in} 运算符都耗时相同。
我会在章节~\ref{hashtable}中解释其原理，暂时这些还不重要。
...
{\bf 实现}是执行计算的方式; 有好有坏。
比如，字典实现的优势在于，我们无需提前知道字符串中存在哪些字母，
我们只需要为每个出现的字母提供空间而已。
...
\end{verbatim}
%
这个函数叫做 {\tt 直方图}，是个统计术语，表示计数(或者频次)的集合.
\index{histogram}
\index{frequency}
\index{traversal}

函数第一行新建了一个空字典。
 {\tt for} 循环便利字符串。每次循环中，如果字符{\tt c}不在字典中，
则创建一个新的项，{\tt c}为建，初始值为1（只遇到了一次）。
...
根据目标不同，可能你只需要选一个，也可能需要将所有的放入列表。
第二，没有一个简单的句法可以{\bf 反向查找}; 你需要确定一下。
...
\end{verbatim}
%
自己创造的异常和Python抛出的异常效果一样：
输出追溯信息以及错误信息。
\index{traceback}
\index{optional argument}
\index{argument!optional}

当你自己抛出异常时，可以将详细错误信息作为可选参数。比如:
...
所以，可能有些字母的频次相同，那么这个倒过来的字典中，每个值都应该是个字母列表。
...
那便创建一个新项，并用{\bf 单元素列表}(包含一个元素的列表) 来初始化它。
如果在字典中，则将此键添加到对应列表中。\index{singleton}
...
通常如果值是整树，浮点数或者字符串，我会将其绘制在箱体内部，
但如果是列表，我会绘制在箱体外部，只是为了简单易懂。
\index{state diagram}
\index{diagram!state}

列表可以是字典的值，就像图中所示，但是字典不能作为键使用。
下面是擅自使用遇到的错误:
\index{TypeError}
\index{exception!TypeError}
...
{\bf hash}是一个函数，其接收任意值(任何类型)，然后返回个整数。
字典使用这些被称为哈希值的整数，来存储和查找键值对。
\index{immutability}
...
如果你已经学习了章节~\ref{one.more.example}中的 {\tt fibonacci} 函数，
你会发现，随着参数越大，函数运行时间越长。同时，时间增长的速度很快。
\index{fibonacci function}
\index{function!fibonacci}
...
此图最顶层，表示 {\tt n=4} 的{\tt fibonacci}函数会调用{\tt n=3}和{\tt n=2}时的
函数。同样， {\tt n=3}时的{\tt fibonacci}函数会调用{\tt n=2} 和 {\tt n=1}时的函数，
以此类推。
\index{function frame}
\index{frame}
\index{call graph}
...
将之前计算过的值存储起来，方便后续使用，这种操作叫做{\bf 备忘}.
...
当你运行这个版本的 {\tt fibonacci}，同时和以前版本比较，
你会发现，快了很多。
...
\end{verbatim}
%
全局变量很有用，但是你使用了很多，并且还频繁修改，那么程序往往会
难以调试。

\section{调试}
\index{debugging}
随着数据集越来越大，仅仅使用打印输出以及手工校验，很难满足需求。
下面是调试大型数据集的一些建议:
...
然后修正异常，并逐步增大{\tt n}值，往复循环。

\item[检验概要及类型:] 不要打印并检验全部数据集，只需输出数据的概要：
比如，字典中项的数量，或者列表中数字之和。

值类型错误，是产生运行时错误的常见原因。
所以调试此类错误，通常打印值的类型即可。

\item[编写自检代码:]  有时候，你可以编写代码，自动检验异常。
例如，你要计算数字列表的平均值，你可以检验一下，结果是不是没有
...
\item[格式化输出:] 格式化调试输出，可以更直观地展示错误。
回忆章节~\ref{factdebug}中的案例。另一个好用的工具是 {\tt pprint} 模块，
...
\item[键(key):] 字典中的一个对象，作为键值对的第一部分。
\index{key}
...
\item[实现(implementation):] 实现计算的方式。
\index{implementation}
...
\item[备忘(memo):] 存储计算过的值，避免后续计算的操作。
\index{memo}
...
\item[全局声明(global statement):] 声明变量是全局变量的语句.
\index{global statement}
\index{statement!global}
...
如果做了习题~\ref{wordlist1}，可以比较一下这种实现，相比列表的{\tt in}运算，
以及二分查找的速度。
...
\begin{exercise}
用字典备忘的方式重写习题~\ref{ackermann} 中的Ackermann函数，
看看是否缓存方式可以使其处理较大参数。
提示: 不能.
...
输入一个列表，如果其中存在任意重复对象，则返回{\tt True}.
...
如果替换首字母，也就是把首字母放回去，移除第二个字母，然后得到的又是
同音词。那么问题来了，这是什么词?
